...
const ProductCard = ({ 
  ...
  <motion.div 
    ...
    whileTap={{ scale: 0.98 }}
    onClick={onClick}
    className="flex flex-col group cursor-pointer relative glass-card p-3 rounded-[1.75rem] border border-white/70 shadow-sm hover:shadow-xl hover:border-primary/30 transition-all duration-300"
  >
    <div className="aspect-square rounded-2xl overflow-hidden mb-3 relative bg-slate-100/60">
      <img 
      ...
      />
      {product.category && (
        <span className="absolute top-2.5 left-2.5 px-2.5 py-1 bg-white/85 backdrop-blur-md text-[10px] font-black uppercase tracking-wider text-primary rounded-full shadow-sm border border-white/80">
          {product.category}
        </span>
      )}
      {isAdmin && onDelete && (
        <button 
          ...
          className="absolute top-2.5 right-2.5 w-8 h-8 bg-red-500/90 hover:bg-red-600 text-white rounded-full shadow-lg flex items-center justify-center transition-all active:scale-90 z-10"
          title="Eliminar Peça"
        >
          <Trash2 size={16} />
        </button>
      ...
      <button 
        ...
        className="absolute bottom-2.5 right-2.5 w-9 h-9 bg-primary text-white rounded-full shadow-lg flex items-center justify-center hover:bg-primary-hover hover:scale-110 transition-all active:scale-90"
    ...
    <div className="px-1 flex flex-col flex-1 justify-between">
      <div>
        <h3 className="text-on-surface font-bold text-sm truncate group-hover:text-primary transition-colors">{product.name}</h3>
      ...
      <div className="flex items-center justify-between mt-2 pt-2 border-t border-slate-100">
        <span className="text-primary font-black text-base tracking-tight">€{product.price.toFixed(2)}</span>
        <span className="text-[10px] font-bold text-primary/70 flex items-center gap-0.5 group-hover:translate-x-0.5 transition-transform">
...
const ShopView = ({ 
  ...
  return (
    <motion.div 
      initial={{ opacity: 0 }}
      animate={{ opacity: 1 }}
      className="pt-24 pb-32 px-4 sm:px-6 max-w-3xl mx-auto"
    >
      <div className="mb-6 flex justify-between items-start">
        <div>
          <h2 className="text-3xl font-black tracking-tight text-on-surface mb-1 font-headline">Catálogo 3D</h2>
          <p className="text-on-surface-variant text-sm font-medium">Peças exclusivas impressas com acabamento premium.</p>
        </div>
        <div className="flex gap-2">
          <button 
            onClick={onRestoreDefaultProducts}
            className="p-2.5 bg-primary/10 hover:bg-primary/20 text-primary border border-primary/20 rounded-2xl shadow-sm active:scale-95 transition-all flex items-center gap-1.5"
            title="Repor Catálogo Inicial"
          >
            <RotateCcw size={18} />
            <span className="text-xs font-bold uppercase tracking-wider hidden sm:inline">Repor Catálogo</span>
          </button>
          {isAdmin && (
            <button 
              onClick={onGoToAdmin}
              className="p-2.5 bg-primary text-white rounded-2xl shadow-lg active:scale-95 transition-all flex items-center gap-1.5"
              title="Painel Admin"
            >
              <Settings size={18} />
      ...
      {/* Category Pills */}
      {categories.length > 1 && (
        <div className="flex gap-2 overflow-x-auto pb-4 mb-6 no-scrollbar pt-2">
          {categories.map(cat => (
            <button
              key={cat}
              onClick={() => setSelectedCategory(cat)}
              className={`px-4 py-2 rounded-full text-xs font-bold whitespace-nowrap transition-all duration-300 shadow-sm ${
                selectedCategory === cat 
                  ? 'bg-primary text-white shadow-md shadow-primary/25 scale-105' 
                  : 'bg-white/80 text-on-surface-variant hover:bg-white border border-slate-200/80'
              }`}
            >
              {cat}
              {cat !== 'Todos' && (
                <span className={`ml-1.5 px-1.5 py-0.5 rounded-full text-[9px] ${selectedCategory === cat ? 'bg-white/20 text-white' : 'bg-slate-100 text-outline'}`}>
      ...
      {products.length === 0 ? (
        <div className="glass-card p-10 rounded-[2.5rem] text-center flex flex-col items-center justify-center space-y-4 my-8">
          <div className="w-16 h-16 bg-primary/10 rounded-2xl flex items-center justify-center text-primary">
            <ShoppingBag size={32} />
          </div>
          <div>
            <h3 className="text-xl font-bold text-on-surface">Nenhuma peça no catálogo</h3>
            <p className="text-sm text-on-surface-variant mt-1 max-w-sm">
              O catálogo de produtos encontra-se vazio. Pode repor os produtos originais com um clique.
            </p>
          </div>
          <button
            onClick={onRestoreDefaultProducts}
            className="px-6 py-3.5 signature-gradient text-white font-bold rounded-2xl shadow-lg hover:shadow-xl active:scale-95 transition-all flex items-center gap-2 text-sm"
          >
            <RotateCcw size={18} />
            Repor Produtos Iniciais
          </button>
        </div>
      ) : (
        <div className="grid grid-cols-2 sm:grid-cols-2 md:grid-cols-3 gap-4">
          {[...filtered].sort((a,b) => (a.order || 999) - (b.order || 999)).map(product => (
            <div key={product.id}>
              <ProductCard 
                product={product} 
      ...
      <footer className="mt-14 mb-8 text-center opacity-40">
        <p className="text-[10px] font-black uppercase tracking-[0.25em]">3D PRODUÇÕES © 2026 - ATELIÊ DIGITAL</p>
      </footer>
    </motion.div>
  );
};
    ...
    >
      <div className="mb-8 flex items-center justify-between">
        ...
        <div className="flex gap-2 items-center">
          ...
          <button 
            onClick={onRestoreDefaultProducts}
            className="p-2 hover:bg-primary/10 rounded-full transition-colors text-primary flex items-center gap-1"
            title="Repor Catálogo Inicial"
          >
            <RotateCcw size={20} />
          </button>
...
export default function App() {
  ...
  const restoreDefaultProducts = async () => {
    setModal({
      show: true,
      title: "Repor Catálogo Inicial",
      message: "Deseja repor o catálogo com os produtos originais da loja 3D Produções?",
      type: 'confirm',
      onConfirm: async () => {
        setStatusMessage("A repor catálogo original...");
        setProducts(PRODUCTS);
        localStorage.setItem('3dproducoes_products', JSON.stringify(PRODUCTS));
        
        if (db) {
          try {
            console.log("Firestore: Repondo produtos iniciais...");
            ...
            console.log("Firestore: Produtos iniciais repostos com sucesso.");
            setStatusMessage("Catálogo original reposto com sucesso!");
          } catch (error) {
            console.error("Erro ao repor catálogo no Firestore:", error);
            setStatusMessage("Catálogo reposto localmente!");
          }
        } else {
          setStatusMessage("Catálogo original reposto com sucesso!");
        }
            ...
            <main className="pb-20">
              {view === 'shop' && (
                <ShopView 
                  ...
                  onRestoreDefaultProducts={restoreDefaultProducts}
                />
              )}
              {view === 'profile' && (
                <ProfileView 
